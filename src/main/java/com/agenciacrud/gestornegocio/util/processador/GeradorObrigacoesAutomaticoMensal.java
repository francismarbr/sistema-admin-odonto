...
public class GeradorObrigacoesAutomaticoMensal extends Thread {


	private final Logger log = LogManager
			.getLogger(GeradorObrigacoesAutomaticoMensal.class);
	...
	//long intervalo = 211600000; // verifica a cada 6h
	long intervalo = 600000; //6 minutos
	
	private List<Agendamento> buscaUltimoRegistro;
	
	public void run() {
		System.out.println("Entrei no método run");
		while (true) {
			try {
				...
				log.error("Gerador de Agendamentos interrompido.", e);
			}
		}
				...
				if (isExecutarGerador()) {
					...
					for (Agendamento agendamento : listaInclusao) {
						...
						if (agendamento.getTipoIntervalo().equals(TipoMensalAnualEnumerador.MENSAL)) {
							...
							agenda.setEmpresa(agendamento.getEmpresa());
							agenda.setDataDoRegistro(new Date());
							agenda.setRegimeTributario(agendamento.getRegimeTributario());
							if(agendamento.getHorario() != "")
								agenda.setHorario(agendamento.getHorario());
